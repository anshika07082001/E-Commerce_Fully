  ...
  return (
    <div className="col-12 d-flex flex-column align-items-center window__Size">
      ...
      <div className="col-10 col-lg-5 col-md-6 col-sm-7 p-3 mb-4 rounded shadow d-flex align-items-center flex-column">
        <h2>Log In</h2>
        <span className={msgFlg.errMsg=="Login Successfully!!"?"text-success":"text-danger"}>{msgFlg.errMsg}</span>
        <div className="d-flex flex-column col-10 p-2">
          <label className="mt-1">Enter Email</label>
          <input className="mt-1 p-1" type='email' ref={(ref)=>inpRefs.current.email=ref}/>
          <label className="mt-1">Enter Password</label>
          <input className="mt-1 p-1" type='password' ref={(ref)=>inpRefs.current.pwd=ref}/>
          {msgFlg.flg?<img className="col-3 m-auto" src="https://i.gifer.com/origin/ec/ecf46fc2a40f43ad0ef438b04b0d2e8e_w200.gif" alt=""/>:<></>}
          <button onClick={logHandler} className="col-5 m-auto mt-3 p-2 rounded border-0 bg-warning">
            Log In
          </button>
        </div>
